...
P_site_mean <- P %>% 
  group_by(Region, site) %>%
  summarise(
    bulk_P = mean(bulk_P_cmgg, na.rm = TRUE),
    algae_P = mean(algae_P_cmgg, na.rm = TRUE),
    .groups = "drop"
  )
...
P_join <- P %>%
  select(Region, site, sediment, bulk_P_cmgg, algae_P_cmgg)
...
CN_molar <- CN %>%
  ...
  mutate(
    ...
    bulk_P_mmolg  = bulk_P_cmgg / 31,
    algal_P_mmolg = algae_P_cmgg / 31,
